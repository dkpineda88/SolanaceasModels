# -*- coding: utf-8 -*-
"""
Created on Tue Mar  3 17:06:29 2026
...
# -*- coding: utf-8 -*-
"""
DeepLabV3+ MobileNetV2 - Potato Disease Detection
OPTIMIZED: menor inferencia + mejor mIoU + INT8 TFLite para Android/Kotlin
Clases: 0:EarlyBlight  1:Healthy  2:LateBlight
"""
...
# ============================================================================
# 1. DATASET CON AUGMENTATION SINCRONIZADO imagen + mascara
# ============================================================================
class PotatoDiseaseDataset(tf.keras.utils.Sequence):
    """
    Augmentation sincronizado: las transformaciones geometricas se aplican
    identicamente a imagen Y mascara para evitar desalineamiento.
    """
    CAT_MAP     = {1: 0, 2: 1, 3: 2}
    HEALTHY_IDX = 1
    ...
    def _augment(self, image, mask):
        ...
        if np.random.rand() > 0.5:
            ...
            for c in range(mask.shape[-1]):
                mask[:,:,c] = cv2.warpAffine(mask[:,:,c], M, self.img_size,
    ...
    def __getitem__(self, index):
        ...
        for img_id in batch_ids:
            ...
            if image is None:
                print(f"  Error cargando: {path}"); continue
    ...
    def summarize_dataset(self):
        ...
        print("\n RESUMEN DEL DATASET:")
        for k, v in counts.items(): print(f"  {k}: {v} imagenes")
        return counts
    ...
    def visualize_categories(self):
        ...
        for i in range(len(self.ids)):
            ...
            if idx not in found: found[idx] = i
            if len(found) == NUM_CLASSES: break
        plt.figure(figsize=(15, 5))
        for i, cls_idx in enumerate(sorted(found.keys())):
            img_info = self.coco.loadImgs(self.ids[found[cls_idx]])[0]
            img = cv2.imread(os.path.join(self.img_dir, img_info['file_name']))
            img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
            plt.subplot(1, NUM_CLASSES, i+1)
            plt.imshow(img)
            plt.title(CLASS_NAMES[cls_idx], fontsize=12, fontweight='bold')
            plt.axis('off')
        plt.tight_layout(); plt.show()


# ============================================================================
# 2. ASPP OPTIMIZADO: dilataciones {6,12,18} + DW-Separable convolutions
#    ~40% menos parametros, ReLU6 para compatibilidad INT8 en Android
# ============================================================================
def _dw_sep_conv(x, filters, dilation_rate=1):
    """Depthwise-Separable + BN + ReLU6. Esencial para cuantizacion INT8."""
...
def ASPP_block(tensor, out_filters=256):
    ...
    b0 = layers.BatchNormalization()(b0); b0 = layers.ReLU(6.0)(b0)
    ...
    bp = layers.BatchNormalization()(bp); bp = layers.ReLU(6.0)(bp)
    bp = layers.UpSampling2D(size=(dims[1], dims[2]),
                              interpolation='bilinear')(bp)
    res = layers.Concatenate()([b0, b1, b2, b3, bp])
    res = layers.Conv2D(out_filters, 1, use_bias=False)(res)
    res = layers.BatchNormalization()(res); res = layers.ReLU(6.0)(res)
    return res


# ============================================================================
# 3. DEEPLABV3+ CON SKIP CONNECTION
#    low_feat (stride 4) fusionado con ASPP -> mejor mIoU en bordes
# ============================================================================
def build_deeplab_plus_mobilenet(input_shape=(256, 256, 3),
    ...
    # Stride 16 -> ASPP
    high_feat = base.get_layer("block_13_expand_relu").output  # 16x16
    # Stride  4 -> skip connection (detalle espacial en bordes)
    low_feat  = base.get_layer("block_3_expand_relu").output   # 64x64
    ...
    low  = layers.BatchNormalization()(low); low = layers.ReLU(6.0)(low)

    x    = layers.Concatenate()([x, low])      # 64x64, fusion DeepLabV3+
    x    = _dw_sep_conv(x, 256)
    x    = _dw_sep_conv(x, 256)

    x    = layers.UpSampling2D(size=(4,4), interpolation='bilinear')(x)
    mask_out = layers.Conv2D(num_classes, 1,
                             activation='softmax', name='mask_out')(x)

    # ---- Classification head -----------------------------------------------
    c5   = base.get_layer("out_relu").output
    gap  = layers.GlobalAveragePooling2D()(c5)
    gap  = layers.Dense(128, use_bias=False)(gap)
    gap  = layers.BatchNormalization()(gap); gap = layers.ReLU(6.0)(gap)
    gap  = layers.Dropout(0.4)(gap)
    class_out = layers.Dense(num_classes, activation='softmax',
                              name='class_out')(gap)

    return models.Model(inputs=base.input,
                        outputs=[mask_out, class_out]), base
...
def compile_model(model, lr):
    model.compile(
        ...
        loss={"mask_out":  focal_tversky_loss,
              "class_out": label_smoothing_cce},
        loss_weights={"mask_out": 1.0, "class_out": 0.5},
        metrics={"mask_out":  [UpdatedMeanIoU(num_classes=NUM_CLASSES,
...
def unfreeze_top_layers(backbone, n=30):
    ...
    print(f"  Descongeladas {n} capas del backbone.")
...
# ============================================================================
print("\n Cargando datasets...")
train_ds = PotatoDiseaseDataset(BASE_PATH+"train",
    BASE_PATH+"train/_annotations.coco.json", BATCH_SIZE, augment=True)
val_ds   = PotatoDiseaseDataset(BASE_PATH+"valid",
    BASE_PATH+"valid/_annotations.coco.json", BATCH_SIZE, augment=False)
test_ds  = PotatoDiseaseDataset(BASE_PATH+"test",
    BASE_PATH+"test/_annotations.coco.json",  BATCH_SIZE, augment=False)
...
# ============================================================================
model, backbone = build_deeplab_plus_mobilenet(freeze_backbone=True)
compile_model(model, lr=1e-4)
model.summary()

# ---- FASE 1: backbone frozen, LR 1e-4 ------------------------------------
cb1 = [
    EarlyStopping(monitor='val_loss', patience=8,
                  restore_best_weights=True, verbose=1),
    ModelCheckpoint('best_phase1_potato.keras',
...
h1 = model.fit(train_ds, validation_data=val_ds,
               epochs=35, callbacks=cb1, verbose=1)

# ---- FASE 2: fine-tuning ultimas 30 capas, LR 2e-5 ----------------------
...
cb2 = [
    EarlyStopping(monitor='val_loss', patience=8,
                  restore_best_weights=True, verbose=1),
    ModelCheckpoint('best_potato_optimized.keras',
                    monitor='val_class_out_accuracy',
                    save_best_only=True, verbose=1),
    ReduceLROnPlateau(monitor='val_loss', factor=0.5,
...
h2 = model.fit(train_ds, validation_data=val_ds,
               epochs=20, callbacks=cb2, verbose=1)

hist = {}
for k in h1.history:
    hist[k] = h1.history[k] + h2.history.get(k, [])
pd.DataFrame(hist).to_csv("history_potato_optimized.csv", index=False)
...
_, recall, f1, _ = precision_recall_fscore_support(
    y_true_list, y_pred_list, average='weighted', zero_division=0)

print("\n" + "="*55)
print("  RESULTADOS FINALES")
print("="*55)
print(f"  Perdida Total:         {results[0]:.4f}")
print(f"  Segmentacion mIoU:     {results[5]*100:.2f}%")
print(f"  Segmentacion Acc:      {results[4]*100:.2f}%")
print(f"  Clasificacion Acc:     {results[3]*100:.2f}%")
...
def plot_training_history(h):
    ...
    for ax, (tr, va, title) in zip(axes, [
        ...
        ax.plot(h[tr], label='Train'); ax.plot(h[va], label='Val')
        ax.set_title(title); ax.legend(); ax.grid(True)
    plt.tight_layout()
    plt.savefig('training_history_potato_optimized.png', dpi=150); plt.show()
...
def plot_confusion_matrix(y_true, y_pred, classes):
    ...
    plt.title('Confusion Matrix (%) - Potato DeepLabV3+ Optimized')
    plt.xlabel('Predicted'); plt.ylabel('True')
    plt.tight_layout()
    plt.savefig('confusion_matrix_potato_optimized.png', dpi=300); plt.show()
...
def visualize_predictions(model, dataset, n=4):
    ...
    for i in range(n):
        ...
        pr_cf = p[1][i][pr_c]
        ax[i,0].imshow(img);                                  ax[i,0].set_title(f"GT: {CLASS_NAMES[gt_c]}", color='blue')
        ax[i,1].imshow(gt_m, cmap='viridis', vmin=0, vmax=2); ax[i,1].set_title("GT Mask")
        ax[i,2].imshow(pr_m, cmap='viridis', vmin=0, vmax=2); ax[i,2].set_title("Pred Mask")
        ax[i,3].imshow(p[0][i,:,:,pr_c], cmap=COLORS[pr_c])
        ax[i,3].set_title(f"PRED: {CLASS_NAMES[pr_c]}\nConf:{pr_cf:.2f}",
                          color='red', fontweight='bold')
        for a in ax[i]: a.axis('off')
    plt.tight_layout()
    plt.savefig('predictions_potato_optimized.png', dpi=150); plt.show()
...
# ============================================================================
# 8. BENCHMARK LATENCIA (Keras + TFLite)
# ============================================================================
def benchmark_latency(predict_fn, test_ds, label="", iterations=200):
    sample, _ = test_ds[0]
    img = np.expand_dims(sample[0], axis=0).astype(np.float32)
    for _ in range(20): predict_fn(img)      # warm-up
    lats = []
    ...
    print(f"  {label:12s}  avg={avg:.1f}ms  std={std:.1f}ms  "
          f"p95={p95:.1f}ms  FPS={1000/avg:.1f}")
    return avg

print("\n BENCHMARK LATENCIA:")
benchmark_latency(lambda img: model.predict(img, verbose=0),
                  test_ds, label="Keras")

total_params = model.count_params() / 1e6
print(f"  Parametros: {total_params:.2f} M")


# ============================================================================
# 9. GUARDADO + TFLite INT8 (optimizado para Android/Kotlin)
# ============================================================================
print("\n Guardando modelo Keras...")
model.save("PotatoDeepMobileNet_Optimized.keras")

def representative_data_gen():
    """200 muestras reales para calibrar rangos de activacion INT8."""
...
# float32 en I/O: Kotlin no necesita dequantizar manualmente
converter.inference_input_type  = tf.float32
converter.inference_output_type = tf.float32

tflite_bytes = converter.convert()
tflite_path  = 'PotatoDeepMobileNet_Optimized_INT8.tflite'
...
print(f"  TFLite INT8: {tflite_mb:.2f} MB  "
      f"(Keras ~{keras_mb:.1f} MB, reduccion {keras_mb/tflite_mb:.1f}x)")
...
benchmark_latency(tflite_predict, test_ds, label="TFLite INT8")
print("  (CPU desarrollo - Android ARM ~2-3x mas rapido con NNAPI/GPU)")

print("\n" + "="*55)
print("  ARCHIVOS GENERADOS")
print("="*55)
print("  PotatoDeepMobileNet_Optimized.keras")
print("  PotatoDeepMobileNet_Optimized_INT8.tflite  <- Android/Kotlin")
print("  best_potato_optimized.keras")
print("  history_potato_optimized.csv")
print("  training_history_potato_optimized.png")
print("  confusion_matrix_potato_optimized.png")
print("  predictions_potato_optimized.png")
print("="*55)
