...
# ============================================================================
# 2. ARQUITECTURA MOBILENETV2 (Optimizado para Móviles)
# ============================================================================
def build_mask_mobileNetV2(input_shape=(256, 256, 3), num_classes=5):
    # Base de MobileNetV2 pre-entrenada
    ...
    # Extraemos la capa de salida de MobileNetV2 (7x7x1280 para una entrada de 224, 8x8x1280 para 256)
    c5 = base_model.get_layer("out_relu").output # Capa final de extracción de características
    
    # --- Branch 1: Classification Head ---
    gap = layers.GlobalAveragePooling2D()(c5)
    class_fc = layers.Dense(512, activation='relu')(gap)
    class_fc = layers.Dropout(0.3)(class_fc)
    class_out = layers.Dense(num_classes, activation='softmax', name="class_out")(class_fc)

    # --- Branch 2: Mask Head (Decoder simétrico) ---
    # Usamos Conv2DTranspose para ir de 8x8 a 256x256
    x = layers.Conv2D(256, (3, 3), padding="same", activation="relu")(c5)
    
    # Upsampling gradual para mayor precisión de máscara
    x = layers.Conv2DTranspose(128, (4, 4), strides=(2, 2), padding="same", activation="relu")(x) # 16x16
    x = layers.Conv2DTranspose(64, (4, 4), strides=(2, 2), padding="same", activation="relu")(x)  # 32x32
    x = layers.Conv2DTranspose(32, (4, 4), strides=(2, 2), padding="same", activation="relu")(x)  # 64x64
    
    # Salto final a 256x256 (puedes usar más capas o UpSampling)
...
model.compile(
    optimizer=tf.keras.optimizers.Adam(learning_rate=1e-4), # LR más bajo para ResNet
    loss={
...
history_df.to_csv("history_maskrcnn_mobilenet.csv", index=False)
...
def plot_confusion_matrix_percent(y_true, y_pred, classes):
    ...
    plt.title('Normalized Confusion Matrix (%) - MobileNetV2', fontsize=14)
...
model.save("TMTMaskMobilNet5Class.keras")
print("✅ Modelo guardado: TMTMaskMobilNet5Class.keras")
...
with open('TMTMaskMobilNet5Class.tflite', 'wb') as f:
