...
Modelo UNet + MobileNetV2 para Segmentación y Clasificación de Eggplant
@author: dkpin
"""
...
# ============================================================================
# 3. ARQUITECTURA (UNET CON BACKBONE MOBILENETV2)
# ============================================================================
def build_unet_mobilenet(input_shape=(256, 256, 3)):
    # Definimos la entrada explícitamente para evitar problemas de nombres
    inputs = layers.Input(shape=input_shape, name="image_input")
    
    base_model = MobileNetV2(input_tensor=inputs, include_top=False, weights="imagenet")
    
    # --- CAPAS DE SALTO (Skip Connections) ---
    # Usamos get_layer con nombres que el error confirmó que existen en tu sistema
    s1 = inputs                                           # 256x256 (Nuestra entrada directa)
    s2 = base_model.get_layer("block_1_expand_relu").output   # 128x128
    s3 = base_model.get_layer("block_3_expand_relu").output   # 64x64
    s4 = base_model.get_layer("block_6_expand_relu").output   # 32x32
    
    # El punto más profundo (Bottom/Bridge)
    bridge = base_model.get_layer("block_13_expand_relu").output # 16x16
    ...
    d1 = decoder_block(bridge, s4, 128) # 16x16 -> 32x32
    d2 = decoder_block(d1, s3, 64)      # 32x32 -> 64x64
    d3 = decoder_block(d2, s2, 32)      # 64x64 -> 128x128
    d4 = decoder_block(d3, s1, 16)      # 128x128 -> 256x256 (S1 es el Input original)

    # --- SALIDAS ---
    # 1. Máscara de Segmentación
    mask_out = layers.Conv2D(NUM_CLASSES_SEG, 1, activation="softmax", name="mask_out")(d4)

    # 2. Clasificación (usando la salida global del MobileNetV2)
...
model.compile(
    ...
    loss_weights={"mask_out": 2.0, "class_out": 0.8},
...
history_df.to_csv("history_unet_mobilnetnet_eggplant.csv", index=False)
...
model.save("EggplantUnetMobilnet.keras")
...
tflite_path  = "EggplantUnetMobilnet.tflite"
