...
def AtrousSpatialPyramidPooling(tensor):
    """Bloque ASPP para capturar contexto multiescala"""
    dims = tensor.shape
    
    # 1. Conv 1x1 estándar
    out_1 = layers.Conv2D(256, kernel_size=1, padding="same", use_bias=False)(tensor)
    out_1 = layers.BatchNormalization()(out_1)
    out_1 = layers.Activation("relu")(out_1)
    
    # 2. Atrous Convolutions (Dilation rates: 6, 12, 18)
    out_6 = layers.Conv2D(256, kernel_size=3, dilation_rate=6, padding="same", use_bias=False)(tensor)
    out_6 = layers.BatchNormalization()(out_6)
    out_6 = layers.Activation("relu")(out_6)
    
    out_12 = layers.Conv2D(256, kernel_size=3, dilation_rate=12, padding="same", use_bias=False)(tensor)
    out_12 = layers.BatchNormalization()(out_12)
    out_12 = layers.Activation("relu")(out_12)
    
    out_18 = layers.Conv2D(256, kernel_size=3, dilation_rate=18, padding="same", use_bias=False)(tensor)
    out_18 = layers.BatchNormalization()(out_18)
    out_18 = layers.Activation("relu")(out_18)
    
    # 3. Image Pooling
    out_pool = layers.GlobalAveragePooling2D()(tensor)
    out_pool = layers.Reshape((1, 1, dims[-1]))(out_pool)
    out_pool = layers.Conv2D(256, kernel_size=1, padding="same", use_bias=False)(out_pool)
    out_pool = layers.BatchNormalization()(out_pool)
    out_pool = layers.Activation("relu")(out_pool)
    out_pool = layers.UpSampling2D(size=(dims[1], dims[2]), interpolation="bilinear")(out_pool)
    
    # Concatenación y salida final del bloque
    res = layers.Concatenate()([out_1, out_6, out_12, out_18, out_pool])
    res = layers.Conv2D(256, kernel_size=1, padding="same", use_bias=False)(res)
    res = layers.BatchNormalization()(res)
    res = layers.Activation("relu")(res)
    return res

def build_deeplabv3_resnet50(input_shape=(256, 256, 3), num_classes=5):
    base_model = tf.keras.applications.ResNet50(input_shape=input_shape, include_top=False, weights='imagenet')
    
    # Extraemos características del bloque 4 (para el ASPP)
    # En ResNet50, conv4_block6_out suele ser la capa elegida para DeepLab
    res_out = base_model.get_layer("conv4_block6_out").output # Tamaño: 16x16
    
    # --- Branch 1: DeepLabV3 Segmentation Head ---
    aspp = AtrousSpatialPyramidPooling(res_out)
    
    # Decoder simplificado (DeepLabV3+)
    x = layers.UpSampling2D(size=(4, 4), interpolation="bilinear")(aspp) # 64x64
    x = layers.Conv2D(48, kernel_size=1, padding="same", use_bias=False)(x)
    x = layers.BatchNormalization()(x)
    x = layers.Activation("relu")(x)
    
    # Upsampling Final a 256x256
    x = layers.UpSampling2D(size=(4, 4), interpolation="bilinear")(x)
    mask_out = layers.Conv2D(num_classes, kernel_size=1, activation='softmax', name="mask_out")(x)
    
    # --- Branch 2: Classification Head ---
    # Usamos la salida final de ResNet (C5) para la clasificación
...
# ============================================================================
iou_metric = tf.keras.metrics.MeanIoU(num_classes=5, name="mean_iou")
...
model.compile(
    ...
    loss_weights={
        "mask_out": 1.5,  # DeepLabV3 es denso, requiere peso alto
        "class_out": 0.5  # Incrementamos este peso para subir el Accuracy de clasificación
    },
    metrics={
...
print(f"Forma de imagen: {x_sample.shape}")
print(f"Forma de máscara: {y_sample['mask_out'].shape}")
print(f"Forma de clase: {y_sample['class_out'].shape}")
print(f"Ejemplo de etiqueta de clase: {y_sample['class_out'][0]}")
print(f"Clase predicha: {np.argmax(y_sample['class_out'][0])}")

# Visualizar distribución de canales en primera máscara
sample_mask = y_sample['mask_out'][0]
print("\nDistribución de canales en máscara de ejemplo:")
for i in range(NUM_CLASSES):
    max_val = np.max(sample_mask[:,:,i])
    sum_val = np.sum(sample_mask[:,:,i])
    print(f"  Canal {i} ({train_ds.class_names[i]}): Max={max_val:.2f}, Sum={sum_val:.0f}")
# ============================================================================
# CALLBACKS
# ============================================================================
...
history_df.to_csv("history_deeplabv3_resnet.csv", index=False)
...
def plot_confusion_matrix_percent(y_true, y_pred, classes):
    ...
    plt.title('Normalized Confusion Matrix (%) - ResNet50', fontsize=14)
...
model.save("TMTDeepResNet5Class.keras")
print("✅ Modelo guardado: TMTDeepResNet5Class.keras")
...
with open('TMTDeepResNet5Class.tflite', 'wb') as f:
    f.write(tflite_model)

print("✅ Modelo TFLite guardado: TMTDeepResNet5Class.tflite")
