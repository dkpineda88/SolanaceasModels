# -*- coding: utf-8 -*-
"""
Created on Sun Mar  8 15:42:00 2026
...
# ============================================================================
class BellPepperMaskResNetDataset(tf.keras.utils.Sequence):
...
# ============================================================================
# 3. ARQUITECTURA MASK-RESNET50V2
# ============================================================================


def build_mask_resnet(input_shape=(256, 256, 3)):
    # Backbone ResNet50V2 preentrenado
    base_resnet = ResNet50V2(input_shape=input_shape, include_top=False, weights="imagenet")
    base_resnet.trainable = True

    # --- EXTRACCIÓN DE CARACTERÍSTICAS (Puntos de conexión de salto) ---
    # ResNet50V2 reduce el tamaño: 256 -> 128 -> 64 -> 32 -> 16 -> 8
    skip_64x64 = base_resnet.get_layer("conv2_block3_1_relu").output
    high_level_features = base_resnet.get_layer("conv4_block6_1_relu").output # 16x16

    # --- RAMA DE CLASIFICACIÓN ---
    gap = layers.GlobalAveragePooling2D()(base_resnet.output)
    class_out = layers.Dense(NUM_CLASSES, activation="softmax", name="class_out")(layers.Dropout(0.3)(gap))

    # --- RAMA DE MÁSCARA (Decodificador con Skip Connections) ---
    # Paso 1: Upsampling de 16x16 a 64x64
    x = layers.UpSampling2D(size=(4, 4), interpolation="bilinear")(high_level_features)
    
    # Paso 2: Fusión con Skip Connection de 64x64
    skip_conn = layers.Conv2D(64, 1, padding="same", use_bias=False)(skip_64x64)
    skip_conn = layers.BatchNormalization()(skip_conn)
    skip_conn = layers.Activation("relu")(skip_conn)
    ...
    # Paso 4: Upsampling final a 256x256
...
# ============================================================================
# 4. ENTRENAMIENTO Y COMPILACIÓN
# ============================================================================
model = build_mask_resnet()
...
train_ds = BellPepperMaskResNetDataset(base_path + "train", base_path + "train/_annotations.coco.json")
val_ds   = BellPepperMaskResNetDataset(base_path + "valid", base_path + "valid/_annotations.coco.json")
test_ds   = BellPepperMaskResNetDataset(base_path + "test", base_path + "test/_annotations.coco.json")
...
print("🚀 Entrenando Mask-ResNet50V2 para Bell Pepper...")
...
history_df.to_csv("history_mask_resnet_pepper.csv", index=False)
...
model.save("PepMaskResnet.keras")
...
tflite_path  = "PepMaskResnet.tflite"
with open(tflite_path, "wb") as f:
    f.write(tflite_model)

print("✅ Modelo TFLite guardado: PepMaskResnet.tflite")
...
plt.title('Confusion Matrix (%)Potato - Mask + RestNet')
