...
Modelo Mask + MobileNetV2 para Segmentación y Clasificación de Eggplant
@author: dkpin
"""
...
# ============================================================================
# ARQUITECTURA MASK-CLASSIFICATION (ESTILO DEEPLAB LITE)
# ============================================================================
def build_mask_mobilenet(input_shape=(256, 256, 3), num_classes_seg=5, num_classes_clf=5):
    # Definimos la entrada
    inputs = layers.Input(shape=input_shape, name="image_input")
    
    # Backbone: MobileNetV2 (Pre-entrenado en ImageNet)
    # Extraemos características de diferentes niveles para la máscara
    base_model = MobileNetV2(input_tensor=inputs, include_top=False, weights="imagenet")
    
    # Capa de alto nivel (semántica fuerte, resolución baja 16x16)
    high_level_features = base_model.get_layer("block_13_expand_relu").output 
    
    # Capa de bajo nivel (detalles espaciales, resolución alta 64x64)
    low_level_features = base_model.get_layer("block_3_expand_relu").output

    # --- RAMA 1: SEGMENTACIÓN (MASK HEAD) ---
    # Refinamiento de características de alto nivel
    x = layers.Conv2D(128, 3, padding="same", activation="relu")(high_level_features)
    x = layers.BatchNormalization()(x)
    x = layers.UpSampling2D((4, 4), interpolation="bilinear")(x) # Subimos a 64x64
    
    # Proyección de características de bajo nivel para igualar canales
    y = layers.Conv2D(32, 1, padding="same", activation="relu")(low_level_features)
    
    # Fusión de niveles (estilo Mask R-CNN / DeepLab)
    combined = layers.Concatenate()([x, y])
    combined = layers.Conv2D(128, 3, padding="same", activation="relu")(combined)
    combined = layers.Conv2D(64, 3, padding="same", activation="relu")(combined)
    
    # Upsampling final a resolución original (256x256)
    mask_out = layers.UpSampling2D((4, 4), interpolation="bilinear")(combined)
    mask_out = layers.Conv2D(num_classes_seg, 1, activation="softmax", name="mask_out")(mask_out)

    # --- RAMA 2: CLASIFICACIÓN (CLF HEAD) ---
    # Usamos la salida final del backbone (8x8)
    avg_pool = layers.GlobalAveragePooling2D()(base_model.output)
    class_out = layers.Dense(128, activation="relu")(avg_pool)
    class_out = layers.Dropout(0.4)(class_out)
    class_out = layers.Dense(num_classes_clf, activation="softmax", name="class_out")(class_out)
...
model = build_mask_mobilenet(
    ...
    num_classes_seg=5, # [Background, Healthy, LeafSpot, Mosaic, Insect]
    num_classes_clf=5
)
...
def combined_seg_loss():
    def loss(y_true, y_pred):
        ...
        tp = tf.reduce_sum(y_true_f * y_pred_f, axis=[1,2])
        fn = tf.reduce_sum(y_true_f * (1 - y_pred_f), axis=[1,2])
        fp = tf.reduce_sum((1 - y_true_f) * y_pred_f, axis=[1,2])
        tvi = (tp + 1e-6) / (tp + 0.7 * fn + 0.3 * fp + 1e-6)
        return 0.4 * tf.reduce_mean(ce) + 0.6 * tf.reduce_mean(1 - tvi)
    return loss
...
model.compile(
    ...
    loss_weights={"mask_out": 1.7, "class_out": 0.8},
...
history_df.to_csv("history_mask_mobilnetnet_eggplant.csv", index=False)
...
model.save("EggplantMaskMobilnet.keras")
...
tflite_path  = "EggplantMaskMobilnet.tflite"
with open(tflite_path, "wb") as f:
    f.write(tflite_model)

print("✅ Modelo TFLite guardado: EggplantUnetMobilnet.tflite")
