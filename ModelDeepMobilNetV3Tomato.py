...
# ============================================================================
# ARQUITECTURA DEEPLABV3 + MOBILENETV3 (Optimizado para Borde/Edge)
# ============================================================================
...
def build_mobilenet_v3_dual(num_classes, img_size=(256, 256)):
    # CORRECCIÓN DEFINITIVA: Desempaquetamos la tupla para obtener enteros
    ...
    x_class = layers.GlobalAveragePooling2D()(base_model.output)
    x_class = layers.Dense(256, activation='relu')(x_class)
    x_class = layers.Dropout(0.3)(x_class)
    class_out = layers.Dense(num_classes, activation='softmax', name='class_out')(x_class)

    # Rama de Segmentación
    # MobileNetV3Large reduce la imagen original 32 veces (256 / 32 = 8)
    x_seg = layers.Conv2D(256, (3, 3), padding='same', activation='relu')(base_model.output)
    # Volvemos al tamaño original (256x256)
    x_seg = layers.UpSampling2D(size=(32, 32), interpolation='bilinear')(x_seg) 
    mask_out = layers.Conv2D(num_classes, (1, 1), activation='softmax', name='mask_out')(x_seg)

    # Definición del modelo con dos salidas
    model = models.Model(inputs=inputs, outputs=[mask_out, class_out])
    return model

# Llamada al modelo con los parámetros globales
model = build_mobilenet_v3_dual(NUM_CLASSES, IMG_SIZE)

# El resto de tu lógica de entrenamiento (callbacks, fit, etc.) se mantiene igual
...
def dice_loss(y_true, y_pred, smooth=1e-6):
    # y_true y y_pred tienen forma (batch, 256, 256, 5)
    y_true_f = tf.reshape(y_true, [-1, 5])
    y_pred_f = tf.reshape(y_pred, [-1, 5])
...
model.compile(
    ...
    loss_weights={
        "mask_out": 2.0,  # DeepLabV3 es denso, requiere peso alto
        "class_out": 0.8  # Incrementamos este peso para subir el Accuracy de clasificación
    },
    metrics={
...
history_df.to_csv("history_deeplabv3_mobilenet7.csv", index=False)
...
def plot_confusion_matrix_percent(y_true, y_pred, classes):
    ...
    plt.title('Normalized Confusion Matrix (%) Tomato - ResNet50', fontsize=14)
...
model.save("TMTDeepMobilNet5Class3.keras")
print("✅ Modelo guardado: TMTDeepMobilNet5Class3.keras")
...
with open('TMTDeepMobilNet5Class3.tflite', 'wb') as f:
    f.write(tflite_model)

print("✅ Modelo TFLite guardado: TMTDeepMobilNet5Class3.tflite")
