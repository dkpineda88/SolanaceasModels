...
# ============================================================================
# 1. DATASET (Con MobileNetV2 Preprocess)
# ============================================================================
        ...
        for img_id in batch_ids:
            ...
            if anns:
                ...
                for ann in anns:
                    ...
                    # --- BLOQUE DE SEGURIDAD ANTICORRUPCIÓN ---
                    if 0 <= m_id < self.num_classes and 'segmentation' in ann:
                        try:
                            # Solo intentamos procesar si la segmentación no está vacía
                            ...
                            # Si la máscara falla, simplemente la ignoramos y pasamos a la siguiente
                            continue
...
# ============================================================================
# 2. ARQUITECTURA: MobileNetV2-UNet Multi-Output
# ============================================================================
def build_mobilenet_unet_multi(input_shape=(256, 256, 3), num_classes=5):
    ...
    class_out = layers.Dense(num_classes, activation='softmax', name="class_out")(layers.Dropout(0.3)(layers.Dense(256, activation='relu')(gap)))
...
# ============================================================================
model = build_mobilenet_unet_multi()
...
train_ds.summarize_dataset(num_samples=5)
...
history_df.to_csv("history_unet_mobilenet.csv", index=False)
...
for i in range(num_samples):
    ...
    axes[i, 2].set_title(f"Prediction segmentation", fontsize=10)
    axes[i, 2].axis('off')
...
def plot_confusion_matrix_percent(y_true, y_pred, classes):
    ...
    plt.title('Normalized Confusion Matrix (%) Tomato - Unet+MobilNetV2', fontsize=14)
...
model.save("TMTUnetMobilNet5Class.keras")
...
with open('TMTUnetMobilNet5Class.tflite', 'wb') as f:
    f.write(tflite_model)

print("✅ Modelo TFLite guardado: TMTUnetMobilNet5Class.tflite")
