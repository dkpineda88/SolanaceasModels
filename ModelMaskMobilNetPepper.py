# -*- coding: utf-8 -*-
"""
Created on Sat Mar  7 21:27:08 2026
...
def build_mask_mobilenet(input_shape=(256, 256, 3)):
    ...
    # Rama de Segmentación (Mask) con Skip-Connection
    
    x = layers.UpSampling2D(size=(4, 4), interpolation="bilinear")(high_level_feat) # 64x64
...
train_ds = BellPepperMaskDataset(base_path + "train", base_path + "train/_annotations.coco.json")
val_ds   = BellPepperMaskDataset(base_path + "valid", base_path + "valid/_annotations.coco.json")
test_ds   = BellPepperMaskDataset(base_path + "test", base_path + "test/_annotations.coco.json")
...
callbacks = [
    ModelCheckpoint("best_mask_pepper.keras", save_best_only=True),
    EarlyStopping(patience=5, restore_best_weights=True),
    ReduceLROnPlateau(factor=0.5, patience=4)
]

print("🚀 Entrenando Mask-MobileNet para Bell Pepper...")
...
history_df.to_csv("history_mask_mobilenet_pepper.csv", index=False)
...
model.save("PepMaskMobilnet.keras")
...
tflite_path  = "PepMaskMobilnet.tflite"
with open(tflite_path, "wb") as f:
    f.write(tflite_model)

print("✅ Modelo TFLite guardado: PepMaskMobilnet.tflite")
...
plt.title('Confusion Matrix (%)Potato - Mask+ MobileNet')
