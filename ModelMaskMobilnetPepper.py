# -*- coding: utf-8 -*-
"""
Created on Tue Mar 10 13:04:55 2026
...
# ============================================================================
# 1. CONFIGURACIÓN (LÓGICA ESTILO TOMATE)
# ============================================================================
IMG_SIZE    = (256, 256)
BATCH_SIZE  = 8

# SEGMENTACIÓN: 3 clases (0=Fondo/Nada, 1=Bacterial, 2=Healthy)
NUM_CLASSES_SEG = 3 
# CLASIFICACIÓN: 2 clases (Bacterial Spot, Healthy)
NUM_CLASSES_CLF = 2
...
# ============================================================================
# 3. MODELO MASK-MOBILENETV2 (ARQUITECTURA)
# ============================================================================
...
def build_mask_mobilenet(input_shape=(256, 256, 3)):
    ...
    # Capas de extracción
    high_level = base_model.get_layer("block_13_expand_relu").output # 16x16
    ...
    # Rama de Máscara (3 clases para incluir el FONDO)
    x = layers.UpSampling2D((4, 4), interpolation="bilinear")(high_level)
    
    low_feat = layers.Conv2D(48, 1, padding="same", activation="relu")(low_level)
    x = layers.Concatenate()([x, low_feat])
    
    x = layers.Conv2D(128, 3, padding="same", activation="relu")(x)
    x = layers.UpSampling2D((4, 4), interpolation="bilinear")(x)
    
    # La salida tiene 3 canales: [Fondo, Bacterial, Healthy]
...
model.save("PepMaskMobilnet3.keras")
...
tflite_path  = "PepMaskMobilnet3.tflite"
with open(tflite_path, "wb") as f:
    f.write(tflite_model)

print("✅ Modelo TFLite guardado: PepMaskMobilnet3.tflite")
...
plt.title('Confusion Matrix (%)Potato - Mask+ Mobilnet')
